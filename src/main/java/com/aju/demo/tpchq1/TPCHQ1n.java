...
//                .keyBy("l_linestatus")
    ...
    public static class Q1AjuAlgo extends
        ...
        private ValueState<RelationsManager> relationsManager;
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            relationsManager = getRuntimeContext().getState(
                    new ValueStateDescriptor<RelationsManager>("relationsManager", RelationsManager.class));
        }
        ...
        public void processElement(
            ...
            RelationsManager currentRelationsManager = relationsManager.value();
            if (currentRelationsManager == null) {
                currentRelationsManager = new RelationsManager();
            }
            currentRelationsManager.setQueryNum(1);

            RelationUnit currentRelation = currentRelationsManager.getRelationMapPool().get("lineitem");
            if(currentRelation == null) {
                throw new RuntimeException("current relation lineitem is null.");
            }
            ...
            if (!currentRelation.isLeaf) {
                ...
                if (currentRelation.s_counter != null) {
                    currentRelation.s_counter.put(value.primaryKeyOrderkeyLinenum, 0);
                } else {
                    throw new RuntimeException("relation s_counter is null.");
                }

                // foreach R_c \in C(R) do
                for (int i = 0; i < currentRelation.childRelationsNum; i++) {
                    currentRelation.indexOfRandRc.get(i).indexRAndRc.put();
                }

                if (currentRelation.childRelations != null) {
                    for (RelationUnit r : currentRelation.childRelations) {
